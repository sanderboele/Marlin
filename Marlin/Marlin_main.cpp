...
bool position_is_reachable(const float target[XYZ]
  ...
  #define WITHINZ(z) ((z) >= Z_MIN_POS - 0.0001 && (z) <= Z_MAX_POS + 0.0001)
    ...
    if (by_probe) {
      ...
      bool good = WITHINZ(target[Z_AXIS]) && (R2 >= sq(float(MIDDLE_DEAD_ZONE_R))) && (R2 <= sq(L1 + L2));
      /*
    ...
    #endif
  #elif ENABLED(DELTA)
    return WITHINZ(target[Z_AXIS]) && HYPOT2(dx, dy) <= sq((float)(DELTA_PRINTABLE_RADIUS));
  #else
    return WITHINZ(target[Z_AXIS]) && WITHINXY(dx, dy);
  #endif
}
  ...
  if (IsRunning() && G0_G1_CONDITION) {
    gcode_get_destination(); // For X Y Z E F
      ...
      if (fast_move) {
        ...
        inverse_kinematics(current_position);
        const float mm_of_travel = HYPOT(
...
inline void gcode_M502() {
  ...
  inline void gcode_M440() {
    ...
    if (!isnan(freq) || !isnan(dca) || !isnan(dcb) || !isnan(dcc)) {
      ...
      if ((!isnan(dca) && FABS(dca) > 1.0) || (!isnan(dcb) && FABS(dcb) > 1.0) || (!isnan(dcc) && FABS(dcc) > 1.0)) {
        SERIAL_ERROR_START();
...
void set_current_from_steppers_for_axis(const AxisEnum axis) {
  ...
  inline bool prepare_kinematic_move_to(const float (&cart)[XYZE]) {
    ...
    // SERIAL_ECHOLNPAIR(" segment_mm=", cartesian_segment_mm);
      ...
      /*
      SERIAL_ECHOLNPGM("Preparing kinematic move with scaling");
      SERIAL_ECHOLNPAIR("segment_length=", segment_length);
      SERIAL_ECHOLNPAIR("inv_segment_length=", inv_segment_length);
      SERIAL_ECHOLNPAIR("_feedrate_mm_s=", _feedrate_mm_s);
      SERIAL_ECHOLNPAIR("inverse_secs=", inverse_secs);
    ...
    // Ensure last segment arrives at target location.
      ...
      if (diff2) {
        ...
        SERIAL_ECHOPAIR(" adiff=", adiff); SERIAL_ECHOPAIR(" bdiff=", bdiff);
        SERIAL_ECHOLNPAIR(" F", (HYPOT(adiff, bdiff) * inverse_secs) * 60);
        SERIAL_EOL();
        safe_delay(5);
        */
      }
